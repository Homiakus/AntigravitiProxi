...
func SOCKS5DialContext(proxyAddr string, timeout time.Duration) func(context.Context, string, string) (net.Conn, error) {
	return func(ctx context.Context, network, address string) (net.Conn, error) {
		...
		if resp[0] != 0x05 || resp[1] != 0x00 {
			return fail(fmt.Errorf("SOCKS5 auth negotiation failed: %v", resp))
		}
		...
		if head[0] != 0x05 || head[1] != 0x00 {
			return fail(fmt.Errorf("SOCKS5 CONNECT failed, reply=%d", head[1]))
		}
		var skip int
		switch head[3] {
		case 0x01:
		...
		case 0x04:
			skip = 16
		default:
			return fail(fmt.Errorf("SOCKS5 invalid ATYP %d", head[3]))
		}
